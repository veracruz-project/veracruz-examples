...
pretty_print_wget "$(wget -q --content-on-error -nv  -O- --post-file=$BUNDLE_PATH/input/yolov3-tiny.cfg --header='Content-Type:application/json' http://${HOSTIP}:${HOSTPORT}/function/${FUNCTION_NAME}/data_file/$(echo -n "/program_data/yolov3.cfg" | base64) 2>&1)"
pretty_print_wget "$(wget -q --content-on-error -nv  -O- --post-file=$BUNDLE_PATH/input/yolov3-tiny.weights --header='Content-Type:application/json' http://${HOSTIP}:${HOSTPORT}/function/${FUNCTION_NAME}/data_file/$(echo -n "/program_data/yolov3.weights" | base64) 2>&1)"

...
if [ ! -e "${BUNDLE_PATH}/detector.wasm" ]; then
		...
		wget -q --content-on-error -nv https://github.com/veracruz-project/video-object-detection/releases/download/${TAG}/bundle_small.tar.gz
		mkdir ${BUNDLE_PATH}
...
YOLOV3_CFG_HASH=$(sha256sum "${BUNDLE_PATH}/yolov3-tiny.cfg" | cut -d " "  -f 1)
YOLOV3_WEIGHTS_HASH=$(sha256sum "${BUNDLE_PATH}/yolov3-tiny.weights" | cut -d " "  -f 1)
JSON_FUNCTION_POLICY=$(cat function_policy_small.json | sed -e "s/PROGRAM_HASH/${PROGRAM_HASH}/g" -e "s/COCO_HASH/${COCO_HASH}/g" -e "s/YOLOV3_CFG_HASH/${YOLOV3_CFG_HASH}/g" -e "s/YOLOV3_WEIGHTS_HASH/${YOLOV3_WEIGHTS_HASH}/g")
...
load_file_veracruz "${FUNCTION_NAME}" "data_file" "/program_data/yolov3.cfg" "${BUNDLE_PATH}/yolov3-tiny.cfg"
load_file_veracruz "${FUNCTION_NAME}" "data_file" "/program_data/yolov3.weights" "${BUNDLE_PATH}/yolov3-tiny.weights"
